    ...
    if re.search(r"@tgbot\.on\(.*pattern=(r|)\".*\".*\)", dosy):
        ...
        if (not type(Pattern) == list) or (len(Pattern) < 1 or len(Pattern[0]) < 1):
            if re.search(r'CmdHelp\(.*\)', dosy):
                ...
                return await event.edit(f'**Modul berhasil dimuat**\n__Untuk mendapatkan informasi tentang perintah dan penggunaan modul__`.perintah {cmdhelp}` __menulis.__')
            else:
                await reply_message.forward_to(PLUGIN_CHANNEL_ID)
            ...
            if re.search(r'CmdHelp\(.*\)', dosy):
                cmdhelp = re.findall(r"CmdHelp\([\"'](.*)[\"']\)", dosy)[0]
                await reply_message.forward_to(PLUGIN_CHANNEL_ID)
                return await event.edit(f'**Modul berhasil dimuat**\n__ntuk mendapatkan informasi tentang perintah dan penggunaan modul__ `.perintah {cmdhelp}` __menulis.__')
            else:
                dosyaAdi = reply_message.file.name.replace('.py', '')
                extractCommands(dosya)
                await reply_message.forward_to(PLUGIN_CHANNEL_ID)
                return await event.edit(f'**Modul berhasil dimuat**\n__tuk mendapatkan informasi tentang perintah dan penggunaan modul__ `.perintah {dosyaAdi}` __menulis.__')
...
async def ptest(event):
    ...
    return await event.edit(f'**Modul berhasil dimuat**\
    \n__Anda dapat bereksperimen dengan modules. Plugin tidak akan berfungsi saat Anda memulai ulang bot.__')
